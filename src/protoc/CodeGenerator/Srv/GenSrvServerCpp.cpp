...
#include "GenSrvServerCpp.h"
#include <fstream>
using namespace std::string_literals;

GenCodeResult GenSrvServerCpp(const std::string &srvFileName, const SrvTrial &vars)
{
...
class TypeTrail:
    ...
    auto SrvServerCppGenerator_py_part2 =
R"(
...
responseVars = [
    ...
    // write to file
    auto GenSrvServerCpp_py = srvFileNamePy + SrvServerCppGenerator_py_part1 + requestVars + responseVars + SrvServerCppGenerator_py_part2;
    std::ofstream ofs("GenSrvServerCpp.py");
    ofs << GenSrvServerCpp_py;
    ofs.close();

    // run python script
    auto systemRes = system("python3 GenSrvServerCpp.py");
    if (systemRes != 0)
        throw std::runtime_error("run GenSrvServerCpp.py failed! file: " __FILE__ " line: "s + std::to_string(__LINE__));
